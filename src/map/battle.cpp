...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case NPC_IGNITIONBREAK:
			...
			break;
		case RK_STORMBLAST:
			skillratio += -100 + (((sd) ? pc_checkskill(sd,RK_RUNEMASTERY) : 0) + sstatus->int_ / 5) * 100;
			skillratio += sstatus->int_ * 30;
			// ATK = [{Rune Mastery Skill Level + (Caster's STR / 6)} x 100] %
			//RE_LVL_DMOD(100);
			break;
		...
		case LG_CANNONSPEAR:
			...
			skillratio += skillratio * (sstatus->str / 30);
			RE_LVL_DMOD(100);
			break;
		case LG_BANISHINGPOINT:
		...
		case LG_OVERBRAND_PLUSATK: // Only Piercing and Swing damage get RE_LVL_DMOD bonus damage
			...
			skillratio += 300 * skill_lv + rnd() % 90 + 10;
			break;
		case LG_EARTHDRIVE:
			if (sd) {
				short index = sd->equip_index[EQI_HAND_L];
...
struct Damage battle_calc_magic_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch(skill_id) {// troca elemento
	case LG_SHIELDSPELL:
		if (skill_lv == 2)
			s_ele = ELE_HOLY;
		break;
			...
			default: {
				...
				switch(skill_id) {
					...
					case WL_CRIMSONROCK:
						...
						//Dano = {[(Nv. da habilidade × 300) × (Nv. de base ÷ 100)] + 1.300}%
					...
					case WL_COMET:
						...
						if (sd && sd->status.party_id) {
							...
							if ((psd = map_id2sd(p_sd[c])) && pc_checkskill(psd, WL_COMET) > 0) {
								//Dano = {[(Nv. da habilidade × 400) × (Nv. de base ÷ 120)] + 2.500}
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch (skill_id) {
		...
		break;
#ifndef RENEWAL
OREELEMENT); //These two are not properties of the weapon based part.
			break;
		case HW_GRAVITATION:
